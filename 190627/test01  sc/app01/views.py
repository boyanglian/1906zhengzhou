...
def abc(request):
    ...
    # nowDT = datetime.timedelta(seconds=20) + datetime.datetime.now()
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if username == 'admin' and pwd == 'admin888':
            ...
            return res  # Set-Cookie: is_login=admin:1hgTgr:8uEbe70F7qecPinqgKn-V3gkXZY; Path=/
...
def loginOut(request):
    ...
    red = redirect('/login')
    # 删除指定键的Cookie
    red.delete_cookie('is_login')  # Set-Cookie: is_login=""; expires=Thu, 01 Jan 1970 00:00:00 GMT; Max-Age=0; Path=/
    return red
...
def article(request):
    # get_signed_cookie 获取加盐后的值
    print(request.get_signed_cookie('is_login', salt=settings.SALT), '*' * 10)  # admin
